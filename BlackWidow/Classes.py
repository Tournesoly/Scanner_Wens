...
class Crawler:
    ...
    def extract_vectors(self):
        ...
        for node in self.graph.nodes:
            if node.value.url != "ROOTREQ":
                ...
                if purl.scheme[:4] == "http" and not node.value.url in added: # 为什么会有后面这个判断
                    vectors.append( ("get", node.value.url) )
                    added.add(node.value.url)

        # FORMS and EVENTS 提取表单和事件
        for edge in self.graph.edges:
    ...
    def attack_event(self, driver, vector_edge):
        ...
        for payload_template in xss_payloads:
            ...
            # Launch!
            follow_edge(driver, self.graph, vector_edge) # 单独一个follow edge有什么效果 主要用于导航到目标页面。

            try:
                if  event.event == "oninput" or event.event == "input":
                    el = driver.find_element(By.XPATH, event.addr) # 找到触发事件的目标元素。
                    el.clear() # 清除目标元素的现有内容
                    el.send_keys(payload) # 向目标元素发送负载
                    el.send_keys(Keys.RETURN) # 模拟回车键
                ...
                if  event.event == "oncompositionstart" or event.event == "compositionstart":
                    ...
                    el.click() # 点击目标元素
                    el.clear() # 清空
                    el.send_keys(payload) # 发送负载
                    el.send_keys(Keys.RETURN) # 模拟回车
        ...
        for parameter in purl.query.split("&"):
            if parameter:
                for payload_template in xss_payloads:
                    ...
                    driver.get(attack_vector) # 导航一下 会触发添加的JavaScript代码是不是 然后下面就可以检测了
    ...
    def fix_form(self, form, payload_template, safe_attack):
        ...
        for parameter in form.inputs: # 遍历表单中的每个输入参数，如果存在类型在 only_aggressive 列表中的输入，设置 need_aggressive 为 True 并跳出循环。
            if parameter.itype in only_aggressive:
                need_aggressive = True
                break

        for parameter in form.inputs:
            (lookup_id, payload) = self.arm_payload(payload_template) # 生成一个负载。 将模版中的 变量 替换为真实的值
            if safe_attack: # 安全攻击模式，只注入负载到类型为
                # SAFE.
            ...
            elif need_aggressive: # 激进模式则包含的输入参数类型更多
                # AGGRESSIVE
    ...
    def inspect_attack(self, vector_edge):
        ...
        # attribute injections 属性注入？？？
        attribute_injects = self.driver.find_elements(By.XPATH, "//*[@jaekpot-attribute]") # 使用 XPath 查找页面中所有带有 jaekpot-attribute 属性的元素。
        for attribute in attribute_injects:
            lookup_id = attribute.get_attribute("jaekpot-attribute")
            successful_xss.add(lookup_id)
            self.reflected_payload(lookup_id, vector_edge) # 这就是在检查xss的输出？

        xsses_json = self.driver.execute_script("return JSON.stringify(xss_array)") # 该数组中包含所有成功注入的 lookup_id。 哦 好像是代码执行的时候会吧id存到这个全局数组里面
    ...
    def execute_path(self, driver, path):
        ...
        for edge_in_path in path:
            method = edge_in_path.value.method # 获取边的方法
            method_data = edge_in_path.value.method_data # 获取边的数据：
            logging.info("find_state method %s" % method)
            if method == "get":
    ...
    def track_form(self, driver, vector_edge):
        ...
        for form_edge in form_edges:
            ...
            for parameter in form.inputs:
                ...
                # 如果输入类型是 text 或 textarea，将追踪器值注入该参数。
                    # Arm the payload
    ...
    def path_attack_form(self, driver, vector_edge, check_edge=None):
        ...
        graph = self.graph
        path = rec_find_path(graph, vector_edge) # 找到从根节点到该结点的路径

        logging.info("PATH LENGTH: " + str(len(path)))
        forms = []
        for edge in path:
            if edge.value.method=="form":
                forms.append(edge.value.method_data) # 将所有表单类型的边添加到 forms 列表中。


        # Safe fix form # 安全修复表单并注入负载
        payloads = self.get_payloads() # 获取注入负载
        for payload_template in payloads:
            ...
            if not execute_result:
                ...
                return False
            if check_edge: # check_edge：可选参数，用于在攻击后进行检查的边
            ...
            if inspect_result:
                ...
                return successful_xss # 为什么返回空的xss集合 感觉该返回inspect result 或者是上面几个的并集


        # Aggressive fix form #激进修复表单并注入负载
        payloads = self.get_payloads()
        for payload_template in payloads:
            for form in forms:
                form = self.fix_form(form, payload_template, False)
            self.execute_path(driver, path)
            if not execute_result: # 这里怎么有个空变量
                logging.warning("Early break attack on " + str(vector_edge))
                return False
            if check_edge:
                logging.info("check_edge defined from tracker " + str(check_edge))
        ...
        return successful_xss # 永远是返回空的xss集合？
    ...
    def attack_ui_form(self, driver, vector_edge):
        ...
        for payload_template in xss_payloads:
            ...
            # Launch!
            follow_edge(driver, self.graph, vector_edge) # 主要用于导航到目标页面。 ？ 不记录返回值

            try:
               for source in ui_form.sources: # 遍历表单的所有源，将负载值注入到每个源中。
                   source['value'] = payload
               ui_form_fill(driver, ui_form) # 填充并提交表单。
            except:
            ...
            inspect_result =  self.inspect_attack(vector_edge)
            if inspect_result:
                successful_xss = successful_xss.union() # 疑似有问题 并了个寂寞
                logging.info("Found injection, don't test all")
                break
    ...
    def attack(self):
        ...
        for edge in self.graph.edges:
            if edge.value.method == "get":
                if not check_edge(driver, self.graph, edge): # 限制次数所在
                    logging.warning("Check_edge failed for in attack phase" + str(edge))
                else:
                    successful = follow_edge(driver, self.graph, edge) # find_state
                    if successful:
                        self.track_form(driver, edge) # 好像是注入
        ...
        quick_xss = self.quick_check_xss(driver, vectors) # 这个是直接导航+检查
    ...
    def next_unvisited_edge(self, driver, graph):
        ...
        if user_url:
            ...
            graph.add(req) # 添加结点 
            graph.connect(self.root_req, req, CrawlEdge(req.method, None, current_cookies), graph.data['prev_edge'] ) # 连接该结点与根节点
            ...
            successful = follow_edge(driver, graph, new_edge) # 如果是get 尝试driver.get返回true，其他则尝试find_state，能从根节点到达edge的话则返回true
            if successful:
                return new_edge
            else:
        ...
        list_to_use = [edge for edge in graph.edges if edge.value.method == "iframe" and edge.visited == False] # 优先处理iframe 添加到待处理列表
        if list_to_use:
            print("Following iframe edge") # 只是print一下？

        # Start the crawl by focusing more on GETs
        if not self.debug_mode: # 如果不在debug模式下，优先处理早期的 GET 请求。
            if self.early_gets < self.max_early_gets:
                print("Looking for EARLY gets")
                print(self.early_gets, "/", self.max_early_gets)
                list_to_use = [edge for edge in graph.edges if edge.value.method == "get" and edge.visited == False] # 啊 这是从iframe改成get了？
            ...
            if self.early_gets == self.max_early_gets: # 当达到最大早期 GET 请求数时，重置图的访问状态。
                print("RESET")
        ...
        if not list_to_use and 'prev_edge' in graph.data: # 是否存在 prev_edge（上一个访问的边）
            prev_edge = graph.data['prev_edge']

            if prev_edge.value.method == "form": # 上一个访问的边是表单
                prev_form = prev_edge.value.method_data
                # print(prev_form)
                # print(prev_form.__hash__())
                # print("FORM TO DO: ")
                if not (prev_form in self.attacked_forms): # 未被攻击过
                    print("prev was form, ATTACK")
                    logging.info("prev was form, ATTACK, " + str(prev_form))
                    # TODO should we skip attacking some elements?
                    self.path_attack_form(driver, prev_edge) # 尝试在路径中的表单上注入恶意负载，
                    ...
                    self.track_form(driver, prev_edge) # 在路径中的表单上注入追踪器以检测表单中的潜在漏洞。
                else:
                ...
                self.events_in_row = 0
        if not list_to_use:
            random_int = random.randint(0,100) # 随机选择不同类型的边（表单、GET 请求或事件）进行处理。
            if not list_to_use:
        ...
        # Try fallback to GET
        if not list_to_use: # 之前没有找到合适的边时，尝试回退到处理 GET
        ...
        for edge in graph.edges: # 最终回退到处理任何未访问的边
            if edge.visited == False:
                if not check_edge(driver, graph, edge):
                    logging.warning("Check_edge failed for " + str(edge))
                    edge.visited = True
        ...
        if self.early_gets < self.max_early_gets:
            # Turn off early search 关闭早期探索模式
