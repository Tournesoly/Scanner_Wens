...
# 通过 WebDriver 向 Chrome 浏览器发送自定义的 DevTools 命令，并获取执行结果。
def send(driver, cmd, params={}):
  resource = "/session/%s/chromium/send_command_and_get_result" % driver.session_id # 构建资源路径。
  url = driver.command_executor._url + resource # 完整的请求 URL。
  body = json.dumps({'cmd': cmd, 'params': params}) # JSON 格式的请求体。
  response = driver.command_executor._request('POST', url, body) # 发送 POST 请求到构建的 URL，并传递请求体。
  if "status" in response:
    logging.error(response)

def add_script(driver, script):
  send(driver, "Page.addScriptToEvaluateOnNewDocument", {"source": script})# 通过 Page.addScriptToEvaluateOnNewDocument 命令插入的脚本会在每次新文档（如页面刷新或导航到新页面）加载时自动执行。
...
def same_origin(u1, u2):
    ...
    return (    p1.scheme == p2.scheme
            and p1.netloc == p2.netloc ) #  比较两个解析后的 URL 的 scheme（协议，如 http 或 https）和 netloc（网络位置，如 example.com:80）
...
def allow_edge(graph, edge):
    ...
    if crawl_edge.method == "get":
        ...
        return not (crawl_edge.method_data.event in ignore) # 如果事件在忽略列表中，则不允许该边；否则允许。
    else:
        logging.info("Unsure about method %s, will allow." % crawl_edge.method)
        return True # 其他方法默认允许

    from_url = graph.nodes[1].value.url # 源节点url？

    parsed_to_url = urlparse(to_url)

    # Relative links are fine. (Not sure about // links)
    if not parsed_to_url.scheme: # 通常不会跳出当前网站的域名范围
        return True

    # If the sceme is javascript we can't know to final destination, so we allow.
    if parsed_to_url.scheme == "javascript": # 大多数情况下不会真正导航到新页面，而是用于在当前页面上触发某些交互
        return True
...
def linkrank(link_edges, visited_list): # 对边（链接）进行排序
    tups = []
    for edge in link_edges:
        url = edge.n2.value.url
        purl = urlparse(edge.n2.value.url)

        queries = len(purl.query.split("&")) # 计算查询参数的数量。
        depth = len(purl.path.split("/")) # 计算路径的深度。
    ...
    tups.sort(key = operator.itemgetter(1)) # 排序关键字的优先级为：是否已访问、路径深度、查询参数数量。
